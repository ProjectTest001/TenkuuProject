...
void	CEnemy::Draw(LPDIRECT3DDEVICE9	pDevice)
{
	...
	if(!CCamera::IsView_CullFrustum(m_Vertex_3d.vtx,1.0f))
	{
		return;
	}
	//マトリックス変数宣言
	...
	//モデルファイルの取得
	LPD3DXMESH	pMesh = CXfile::GetMesh(ROBO_ENEMY_ID);
	LPD3DXBUFFER	pBuffer = CXfile::GetBuffer(ROBO_ENEMY_ID);
	DWORD	pWord = CXfile::GetWord(ROBO_ENEMY_ID);

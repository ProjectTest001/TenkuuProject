...
HRESULT	CGame::Init()
{
	...
	CCamera::Init(CCamera::PLAYER_CAMERA);
		...
		m_pPlayer[0]->Init(m_pRenderer->GetDevice(),D3DXVECTOR3(49.0f,15.0f,30.0f),D3DXVECTOR3(0.0f,-0.5f,0.0f),WHITE_TEXTURE);
		...
		for(int	i = -50;i < 50 ; i++)
		{
			for(int	x = -50;x < 50 ; x++)
			{
				CBuilding::Create(m_pRenderer->GetDevice(),D3DXVECTOR3(0.0f,8.0f,-10.0f),D3DXCOLOR(1,1,1,1),D3DXVECTOR3(2.0f,3.0f,2.0f),TREE_TEXTURE);
			}
		}
